...
fn receive<T: SendableType>(mut socket: Socket, receive_channel_tx: Sender<(SocketAddrV4, T)>) {
    ...
    loop {
        ...
        pending_buffer.push_str(&String::from_utf8_lossy(&receive_buffer[..count]));
        
        while let Some(i) = pending_buffer.find(DELIMITER) {
            let drain: String = pending_buffer.drain(..i).collect();
            pending_buffer.drain(..DELIMITER.len());
