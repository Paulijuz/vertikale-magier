...
fn choose_direction(elevator: &Elevator) -> (Direction, States) {
    match elevator.direction {    
        Direction::Up => {
            return 
                if elevator.orders_above() { (Direction::Up, States::Moving) }
                else if elevator.orders_here() { (Direction::Down, States::DoorOpen) }
                else if elevator.orders_below() { (Direction::Down, States::Moving) }
                else { (Direction::Stopped, States::Idle) }
        }
        Direction::Down => {
            return 
                if elevator.orders_below() { (Direction::Down, States::Moving) }
                else if elevator.orders_here() { (Direction::Up, States::DoorOpen)}
                else if elevator.orders_above() { (Direction::Up, States::Moving) }
                else { (Direction::Stopped, States::Idle) }
        }
        Direction::Stopped => {
            return 
                if elevator.orders_here() { (Direction::Stopped, States::DoorOpen) }
                else if elevator.orders_above() { (Direction::Up, States::Moving) }
                else if elevator.orders_below() { (Direction::Down, States::Moving) }
                else { (Direction::Stopped, States::Idle) }
        }
...
fn start_moving(elevator: &mut Elevator, elevio_elevator: &e::Elevator, timer_channel_tx: &Sender<()>) {
